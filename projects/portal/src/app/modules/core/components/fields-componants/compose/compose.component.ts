...
export class ComposeComponent implements FieldDataInterface, OnInit, OnChanges {
  ...
  validation() {
    switch (this.field.validation.operator) {
      case 'requiredOr':
        if (this.field.validation.fields.length > 0) {
          let valueExist: Boolean = false;
          for (let index = 0; index < this.field.validation.fields.length; index++) {
            const element = this.field.validation.fields[index];
          ...
          if (valueExist) {
            ...
            this.form.get(this.field.key).setErrors({'invalid': 'fill one of the fields '});
